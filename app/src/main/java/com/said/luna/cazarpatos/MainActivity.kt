...
class MainActivity : AppCompatActivity() {
    ...
    private var countDownTimer = object : CountDownTimer(10000, 1000) {
        ...
        override fun onFinish() {
            ...
            val patosCazados = textViewCounter.text.toString().toInt()

            procesarPuntajePatosCazados(nombreJugador, patosCazados) // Firestore
    ...
    fun procesarPuntajePatosCazados(nombreJugador: String, patosCazados: Int) {
        ...
        db.collection("ranking")
            ...
            .addOnSuccessListener { documents ->
                if (documents != null && documents.documents != null && documents.documents.count() > 0) {
                    val idDocumento = documents.documents[0].id
                    val jugadorLeido = documents.documents[0].toObject(Player::class.java)
                    ...
                    if (jugadorLeido!!.huntedDucks < patosCazados) {
                        ...
                        Log.w(EXTRA_LOGIN, "No se actualizó puntaje, por ser menor al actual")
                    }
                } else {
